 ...
 * @author lijie
 * @version 1.00
 * @Description: 聊天GUI界面 + 发送 + 记录 + 清屏 + 抖动 + 优化发送和发送人
...
//                shake();
                try {
                    send(new byte[]{-1},tf.getText().trim());
                } catch (IOException e1) {
                    e1.printStackTrace();
                }
...
//                if(e.isControlDown() && e.getKeyCode() == KeyEvent.VK_ENTER){
    ...
    /**
     * @description 发送
     * 此方法需要优化, 聊天记录先后顺序及高并发.
     */
    private void send() throws IOException {
        String msg = sendText.getText();    // 获取发送区域的内容
        String ip = tf.getText().trim();               // 获取 IP 地址
        ip = ip.length() == 0 ? "255.255.255.255" : ip;
